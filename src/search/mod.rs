...
impl SearchEngine {
        ...
        while i + pattern_chars.len() <= text_chars.len() {
            ...
            if matched {
                if self.options.whole_word {
                    let before_ok = i == 0
                        || !text_chars[i - 1].is_alphanumeric() && text_chars[i - 1] != '_';
    ...
    fn find_regex(&self, text: &str, pattern: &str) -> Vec<SearchMatch> {
        let flags = if self.options.case_sensitive { "" } else { "(?i)" };
    ...
    pub fn replace_current(&self, _text: &str, replacement: &str) -> Option<(usize, usize, String)> {
